...
export class ItemWrapper extends MyObject3D {
  ...
  protected _update():void {
    ...
    const kake = Util.instance.mix(1.25, 1.6, this._noise);
    this._itemWrapperA.position.x = Util.instance.mix(0, this.itemSize * 0, this.cutRateA) + Util.instance.map(this.cutRateB, 0, 1, 0, this.itemSize * kake);
    this._itemWrapperB.position.x = this._itemWrapperA.position.x * -1;

    const ang = Util.instance.mix(60, 150, this._noise);
