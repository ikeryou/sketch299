...
export class Visual extends Canvas {
  ...
  protected _update(): void {
    ...
    this._scrollItem.scale.set(itemSize * 2, itemSize * 2, 1);
    this._scrollItem.rotation.y = Util.instance.radian(90);
    const kake = Func.instance.val(2, 1)
    this._scrollItem.position.z = Util.instance.map(sRate, 0, 1, moveDist, -moveDist * kake);
    this._scrollItem.position.y = this._scrollItem.scale.y * 0.5 - itemSize * 0.5

    const totalWidth = (this._item[0].itemSize) * this._item.length - (this._item[0].itemSize * 0.25);

    this._item.forEach((val,i) => {
      val.position.z = i * (val.itemSize * -1.25) + totalWidth * 0.5 + val.itemSize * 0.5;

      const d = (this._scrollItem.position.z - val.itemSize * 0.5) - (val.position.z + val.itemSize * 0.5);
      val.cutRateA = Util.instance.map(d, val.itemSize * 0.5 - val.itemSize, val.itemSize * 0.5, 1, 0);

      // 切った後
      val.cutRateB = Easing.instance.inOutSine(Util.instance.map(d, val.itemSize * 0.5 - val.itemSize * 10, val.itemSize * 0.5 - val.itemSize, 1, 0));
